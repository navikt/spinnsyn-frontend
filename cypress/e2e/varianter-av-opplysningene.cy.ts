describe('Tester logikk i behandling.tsx', () => {
    it('Automatisk behandlet', () => {
        cy.visit(
            'http://localhost:8080/syk/sykepenger?id=4e044d03-2dfe-45e9-a904-77777777'
        )
        cy.injectAxe()

        cy.get('.behandling > .navds-heading').should(
            'have.text',
            'Søknaden er behandlet automatisk'
        )
        ...
        cy.get('.navds-accordion__item.ekspanderbar.ugyldig').should(
            'not.exist'
        )
        cy.checkA11y()
    })
    ...
    it('Manuelt behandlet', () => {
        cy.visit(
            'http://localhost:8080/syk/sykepenger?id=99f389f2-0084-481b-bed8-47f6ac3491d4'
        )
        cy.injectAxe()
    ...
    it('Automatisk behandlet annullert vedtak', () => {
        ...
        cy.get('.behandling > .navds-heading').should(
            'have.text',
            'Søknaden ble behandlet automatisk'
        )
        cy.get('.behandling').contains(
    ...
    it('Manuelt behandlet revurdert vedtak', () => {
        cy.visit(
            'http://localhost:8080/syk/sykepenger?id=9ae82dd2-dcf1-4c16-9e12-35cb6d634338'
        )
        cy.injectAxe()
        ...
        cy.get('.navds-alert').should(
            'not.contain',
            'Du vil motta et eget brev med det nye vedtaket.git o'
        )
    ...
    it('Revurdert vedtak med direkte utbetaling', () => {
        cy.visit(
            'http://localhost:8080/syk/sykepenger?id=21eac584-d8ea-4e4b-bf9a-ae0a400009c4'
        )
        cy.injectAxe()
    ...
    it('Revurdert vedtak med kombinasjonsutbetaling', () => {
        cy.visit(
            'http://localhost:8080/syk/sykepenger?id=85f25c03-faa8-4a99-8f15-971e9406f64f'
        )
        cy.injectAxe()
