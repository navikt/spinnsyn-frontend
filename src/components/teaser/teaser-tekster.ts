const TeaserTekster = {
    'spinnsyn.teaser.tittel': 'Behandlet søknad om sykepenger',
    'spinnsyn.teaser.sykmeldt-fra': 'Sykmeldt fra %ARBEIDSGIVER%',
    'spinnsyn.teaser.uleste': 'Nye',
    'spinnsyn.teaser.leste': 'Tidligere',
    'spinnsyn.sendt.til.nav': 'Sendt til NAV',
    'spinnsyn.sendt.til.arbeidsgiver': 'Sendt til arbeidsgiver',
    'spinnsyn.teaser.annullert': 'Behandlet på nytt',
    'spinnsyn.teaser.annullert.tittel': 'Søknaden behandlet på nytt'
}

export default TeaserTekster
