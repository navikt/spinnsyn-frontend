...
const handler = beskyttetApi(async(req: NextApiRequest, res: NextApiResponse) => {
    ...
    if (spinnsynFrontendInterne()) {
        ...
        const oboSpinnsynBackend = await getOboAccessToken(req.headers.authorization?.split(' ')[ 1 ], serverRuntimeConfig.spinnsynBackendClientId)
        vedtakene = await hentVedtakFraSpinnsynBackendForInterne(oboSpinnsynBackend, sykmeldtFnr!)
    } else {
