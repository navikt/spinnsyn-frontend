...
import { spinnsynFrontendArkivering } from '../../../utils/environment'
...
export const getServerSideProps: GetServerSideProps<VedtakArkiveringProps> = async (ctx) => {
    try {
        ...
        const token = await requestAzureClientCredentialsToken(getServerEnv().SPINNSYN_BACKEND_CLIENT_ID!)
        if (!token.ok) {
        ...
        ctx.res.setHeader('x-nais-app-image', getServerEnv().NAIS_APP_IMAGE ?? '')
